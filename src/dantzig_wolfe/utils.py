...
def is_optimal(total_reduced_cost: float, tol: float = 1e-6) -> bool:
    """
...
def validate_problem_structure(
    c: npt.ArrayLike, F: npt.ArrayLike, client_blocks: list[dict[str, npt.ArrayLike]]
) -> bool:
    """
    ...
    if len(all_indices) != n:
        raise ValueError(
            f"Client blocks cover {len(all_indices)} variables but c has {n} elements"
        )
...
def verify_solution(
    ...
    dict
        ...
        - 'violations': dict
    """
    result = {"is_feasible": True, "obj_value": np.dot(c, x), "violations": {}}
